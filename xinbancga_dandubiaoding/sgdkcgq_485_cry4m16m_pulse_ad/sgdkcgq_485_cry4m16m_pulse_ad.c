...
unsigned long temp_for_400_255 =0;
//
...
{    
    ...
    for(;;)
    {
		...
		if (ad_value_flag ==1)
		{
			...
			//.ms
			m_mcur.ms = pulse_value_handling(m_mcur_ad.ms);
...
unsigned short pulse_value_handling(unsigned short val)
{
	...
	#if USED_SMOOTHING == TRUE
	{
		...
		/*
			...
			//tmp = m_mcur_ad_value * 9 /10 +	m_mcur_ad.ms /10;
		...
		tempLong = 	val ;
		m_mcur_ad_value.ml =( m_mcur_ad_value.ml * 9 /10 )+	(tempLong /10);
		val = m_mcur_ad_value.ms[0];
		return val;
	}
	#else
	{
